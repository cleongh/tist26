...
from ..domain import RuleLayer, Rule, RuleOverride
...
class RuleRegistry:
    ...
    def load_legacy_rules(self, rule_files: List[Path] = None) -> int:
        ...
        if rule_files is None:
            rule_files = self._get_default_rule_files()
    ...
    def _get_default_rule_files(self) -> List[Path]:
        """Get default rule files to load."""
        rule_files = [
    ...
    def get_combined_rules_content(self) -> str:
        ...
        lines = [asp_converter.rule_header_to_asp("Combined dynamic rules from RuleRegistry")]
        
        for layer in [RuleLayer.UNIVERSAL, RuleLayer.LEARNED, RuleLayer.STORY]:
            layer_rules = [r for r in self.get_active_rules(layer) if not r.is_file]
            if layer_rules:
                lines.extend(asp_converter.combined_rules_to_asp(layer_rules, layer.name))
